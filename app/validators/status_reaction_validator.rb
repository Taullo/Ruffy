...
class StatusReactionValidator < ActiveModel::Validator
  SUPPORTED_EMOJIS = Oj.load_file(Rails.root.join('app', 'javascript', 'flavours', 'aether', 'features', 'emoji', 'emoji_map.json').to_s).keys.freeze

  LIMIT = [1, (ENV['MAX_REACTIONS'] || 10).to_i].max
